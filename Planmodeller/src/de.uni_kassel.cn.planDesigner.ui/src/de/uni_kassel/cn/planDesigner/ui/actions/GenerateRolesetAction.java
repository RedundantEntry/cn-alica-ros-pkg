...
public class GenerateRolesetAction implements IObjectActionDelegate {
	...
	private void generateRoleSet(IProgressMonitor monitor, MultiStatus status) {
		...
		ResourceSet localResourceSet = new AlicaResourceSet();
		...
		if(shouldGenerate){
			try{
				String container = PlanDesignerActivator.getDefault().getPreferenceStore().getString(IEditorConstants.PREF_ROLE_DEFINITION_CONTAINER);
