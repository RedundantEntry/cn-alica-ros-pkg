...
package de.uni_kassel.cn.planDesigner.ui.figures;
	...
	public IFigure getNameHolder(){
		if (this.nameHolder == null) {
			...
			if (((Plan)planEditPart.getModel()).isMasterPlan()) {
				nameHolder.setBackgroundColor(plugin.getColorRegistry().get(IEditorConstants.MASTER_PLAN_LABEL_BACKGROUND_COLOR));
			} else {	
				nameHolder.setBackgroundColor(plugin.getColorRegistry().get(IEditorConstants.PLAN_LABEL_BACKGROUND_COLOR));
			}
