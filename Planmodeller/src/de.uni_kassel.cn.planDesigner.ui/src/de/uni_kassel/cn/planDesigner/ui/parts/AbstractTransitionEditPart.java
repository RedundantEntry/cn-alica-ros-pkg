...
import de.uni_kassel.cn.planDesigner.ui.adapter.PropertySourceAdapter;
...
public abstract class AbstractTransitionEditPart extends
	...
	/**
	 * Finds the content figure from the plan. This figure can be passed to
	 * ShortestPathConnectionRouter. 
	 * @return
	 */
	private IFigure getPlanFigure() {
		// Get the plan figure which contains all other figures
		PlanEditor editor = PlanEditorUtils.getPlanEditor(this);
		Plan p = editor.getPlan();
		GraphicalEditPart part =  (GraphicalEditPart)getViewer().getEditPartRegistry().get(p);
		
		
		return part.getContentPane();
	}
