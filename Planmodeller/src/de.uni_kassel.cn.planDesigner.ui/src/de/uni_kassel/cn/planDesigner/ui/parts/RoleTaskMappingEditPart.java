...
import de.uni_kassel.cn.planDesigner.ui.figures.GradientFigure;
import de.uni_kassel.cn.planDesigner.ui.policies.TaskPriorityDirectEditPolicy;
...
import de.uni_kassel.cn.planDesigner.ui.util.TaskPriorityDirectEditManager;
...
//		main.setBackgroundColor(ColorConstants.red);
	...
	private Color computeColor() {
		...
		if(prio == 0.5)
			result =  reg.get(IEditorConstants.PRIORITY_DEFAULT_COLOR);		
		else if(prio < 0)
			result = reg.get(IEditorConstants.PRIORITY_0_COLOR);
		else if(prio < 0.5)
			result = reg.get(IEditorConstants.PRIORITY_1_COLOR);
		else 
			result = reg.get(IEditorConstants.PRIORITY_2_COLOR);
