...
public class DUtilityModeExpressionTreeNode extends AbstractNoChildrenEMFTreeNode
{
	...
	{
		UtilityModeExpression idt = (UtilityModeExpression) getModelObject();
	...
	public String getTask()
	{
		...
		try
		{
			task = (String) getEditingDomain().runExclusive(new RunnableWithResult.Impl<String>()
			{
				public void run()
				{
					EntryPoint entryPoint = ((UtilityModeExpression) getModelObject()).getEntryPoint();
					if (entryPoint == null)
					{
						setResult(null);
						return;
	...
	private EntryPoint internalGetEntryPoint()
	{
		...
		try
		{
			t = (EntryPoint) getEditingDomain().runExclusive(new RunnableWithResult.Impl<EntryPoint>()
			{
				public void run()
				{
					EntryPoint t = ((UtilityModeExpression) getModelObject()).getEntryPoint(); // TODO: EntryPoint vs Task
					if (t == null)
					{
						setResult(null);
						return;
	...
	{
		Command uc = SetCommand.create(getEditingDomain(), getModelObject(), AlicaPackage.eINSTANCE.getUtilityModeExpression_EntryPoint(), value);
	...
	public String getVariable()
	{
		...
		try
		{
			variable = (String) getEditingDomain().runExclusive(new RunnableWithResult.Impl<String>()
			{
				public void run()
				{
					String t = ((UtilityModeExpression) getModelObject()).getVariable();
					setResult(t);
				}
			});
		} catch (InterruptedException e)
	...
	public void setVariable(Object value)
	{
		...
		{
			Command uc = SetCommand.create(getEditingDomain(), getModelObject(), AlicaPackage.eINSTANCE.getUtilityModeExpression_Variable(), value);
			UtilityRepositoryUtils.execCommandAndSaveResource(uc, getCommandStack(), getEditingDomain());
		}
	}
}
