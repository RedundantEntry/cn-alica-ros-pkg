...
public class ZoomToolEntry extends ToolEntry {
	...
	public ZoomToolEntry(String label, String shortDesc) {
		super(
			label,
			shortDesc,
			ImageDescriptor.createFromImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_ZOOM_IN_OUT_16)), // these aren't the ones we want but leave them as placeholders
			ImageDescriptor.createFromImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_ZOOM_IN_OUT_24)));
