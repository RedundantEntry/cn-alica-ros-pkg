...
//				return null;
	...
	private void prepareTree(Tree tree){
		...
		for(EObject eo : usages.keySet()){
			...
			behaviourConfigItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_BEHAVIOUR_CONFIGURATION_16));
			...
			for(Plan p : planStateMapping.keySet()){
				...
				planItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_PLAN_16));
				...
				for(State s : planStateMapping.get(p)){
					...
					stateItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_STATE_16));
