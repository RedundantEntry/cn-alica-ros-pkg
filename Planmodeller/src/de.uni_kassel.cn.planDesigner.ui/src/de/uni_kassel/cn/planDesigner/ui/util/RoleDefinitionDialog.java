	...
	private class RoleLabelProvider implements ILabelProvider{
		...
		public Image getImage(Object element) {
			if(element instanceof Role)
				return PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_ROLE_16);
			else if(element instanceof Characteristic)
				return PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_CHARACTERISTIC_16);
			else
...
//			valueCombo.setText("");
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_ROLE_DEFINITION_16));
	}
