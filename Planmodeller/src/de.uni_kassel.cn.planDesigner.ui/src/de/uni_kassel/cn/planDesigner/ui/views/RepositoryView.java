...
package de.uni_kassel.cn.planDesigner.ui.views;
...
import de.uni_kassel.cn.planDesigner.ui.actions.DeleteBehaviourConfigurationAction;
import de.uni_kassel.cn.planDesigner.ui.actions.DeleteTaskFromRepositoryAction;
import de.uni_kassel.cn.planDesigner.ui.adapter.MultiObjectNotificationAdapter;
...
import de.uni_kassel.cn.planDesigner.ui.util.SelectionProviderMediator;
import de.uni_kassel.cn.planDesigner.ui.wizards.PMLPlanTypeConfigurationWizard;
import de.uni_kassel.cn.planDesigner.ui.wizards.PMLPlanningProblemConfigurationWizard;
...
public class RepositoryView extends ViewPart implements ICommandStackTabbedPropertySheetPageContributor{
	...
	private class TaskrepositoryLabelProvider extends LabelProvider{
		...
		public Image getImage(Object element) {
			Image img = null;
			if(element instanceof Task){
				img = PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_TASK_16);
			}else
	...
	private class BehaviourTreeLabelProvider implements ILabelProvider{
		...
		public Image getImage(Object element) {
			if(element instanceof Behaviour)
				...
				return PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_BEHAVIOUR_CONFIGURATION_16);
			else 
				return PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_UNKNOWN_TYPE);
		}
	...
	private class ViewLabelProvider extends LabelProvider {
		...
		public Image getImage(Object obj) {
			...
			if(obj instanceof Plan)
			{
				...
				{
					return reg.get(IEditorConstants.ICON_MASTER_PLAN_16);	
				}
				else
				{
					return reg.get(IEditorConstants.ICON_PLAN_16);	
				}
			}
			else if(obj instanceof PlanType)
			{
				return reg.get(IEditorConstants.ICON_PLANTYPE_16);
			}
			else if(obj instanceof PlanningProblem)
			{
				return reg.get(IEditorConstants.ICON_PLANNING_PROBLEM_16);
			}
			else
			{
				return reg.get(IEditorConstants.ICON_UNKNOWN_TYPE);
			}
		}
	}
...
//				updateGlobalActions(null);
		...
		tabItem.setImage(PlanDesignerActivator.getDefault().getImageRegistry().get(IEditorConstants.ICON_TASK_16));
	...
	private void hookContextMenu() {
		...
		getSite().registerContextMenu(IEditorConstants.PML_REPOSITORY_ID +".repositoryContextMenu", menuMgr, getSite().getSelectionProvider());
	}
