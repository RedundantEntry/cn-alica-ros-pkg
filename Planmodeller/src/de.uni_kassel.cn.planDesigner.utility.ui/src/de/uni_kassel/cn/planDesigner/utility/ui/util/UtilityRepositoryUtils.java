...
public class UtilityRepositoryUtils
{
	...
	public static Utility getUtility(final long parseLong)
	{
		final UtilityRepository repo = getRepository(UtilityActivator.getDefault().getEditingDomain());
		...
		Utility util = (Utility)new EMFRunnable()
		{
			...
			{
				EList<Utility> utils = repo.getUtilities();
				for(Utility u : utils)
				{
					if(u.getId() == parseLong)
					{
						return u;
					}
				}
